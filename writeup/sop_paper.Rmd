...
kail.fun <- function (age, a, b, c) {
  a + b * exp(-c * age)
}

get.kail.rts <- function (n.sims, n.ops, age, a, b, c) {
  lmean <- log(kail.fun(age, a, b, c))
...
acc.fun <- function(age, alpha, beta) {
  inv.logit(alpha + beta * age)
}

get.logit.acc <- function(n.sims, n.ops, age, alpha, beta) {
  rbinom(n.sims, n.ops, acc.fun(age, alpha, beta)) == n.ops
}
```
...
Where Y is the predicted variable, $a$ is the eventual (adult) asymptote, $b$ is the multiplier for the (infant) intercept, $c$ is the rate of development, and $i$ is age. To illustrate this model, we replot the slowing function from @kail1991 in Figure 2, but extend the function into infancy (with $a = 1$, $b = 5.16$, and $c = .21$).
...
d <- data.frame(x = age_years,
                y = kail.fun(age_years, a = a.samp, b = b.samp, c = c.samp))
...
d <- data.frame(x = ages_months,
                y = inv.logit(alpha.samp + beta.samp * ages_months))
qplot(x, y, geom=c("line"), 
      data=d) + 
  xlab("Age (months)") + 
  ylab("Accuracy") + 
...
So now let's generalize these functions so that we can look at their joint effects on performance, and let's look in infancy.
...
sims <- expand.grid(chain.len = chain.lens, 
  ...
  do(data.frame(rt = get.kail.rts(n.sims = n.sims, n.ops = .$chain.len, 
                                  age = .$age/12, 
                                  a = .5, b = 3, c = .2), 
                success = get.logit.acc(n.sims = n.sims, n.ops = .$chain.len, 
                                         age = .$age, 
                                         alpha = -2, beta = .3))) %>%
  mutate(relevant = rt < theta & success)
...
```{r}
d.wr <- read.csv("../data/word_recognition_MA.csv") %>%
...
```{r}
adult.rt <- d.wr.adult$rt

kail.mse <- function(x) {
  b = x[1]
  c = x[2]
  pred <- kail.fun(d.wr.kid$age_years, adult.rt, b, c)
  err <- mean((d.wr.kid$x_1 - pred)^2) # * d.wr.kid$weight)
  return(err)
}

opt <- optim(c(1,1.5), kail.mse)
```

```{r}
# (Using $1/n$ as the weighting term on the error, like a weighted least-squares). 
# we're not weighting because it broke the half-logit

d.wr.kid$pred <- kail.fun(d.wr.kid$age_years, a = adult.rt, b = opt$par[1], c = opt$par[2])

d_pred <- data.frame(age_years = seq(0,5,.1),
                     rt_pred = adult.rt + opt$par[1] * exp(-opt$par[2] * xs))

ggplot(d.wr.kid, aes(x = age_years, y = x_1)) + 
  geom_point(aes(size = n_1, col = short_cite)) + 
  geom_hline(aes(yintercept=adult.rt), lty=3) +  
  geom_line(data=d_pred, aes(x = age_years, y = rt_pred), col="black", lty=2) +
...
$a = `r adult.rt`$, $b = `r opt$par[1]`$, and $c = `r opt$par[2]`$

Accuracy. 

```{r}
acc.wr.mod <- glm(accuracy ~ age_years, #weights = weight, 
                  family = binomial(psyphy::mafc.logit(2)),
                  data = d.wr.kid)

d_pred$acc_pred <- predict(acc.wr.mod, newdata = d_pred, type = "response")

ggplot(d.wr.kid, aes(x = age_years, y = accuracy)) + 
  geom_point(aes(size = n_1, col = short_cite)) + 
  geom_line(data = d_pred, aes(x = age_years, y = acc_pred), 
            lty = 2) + 
#   geom_smooth(aes(weight = weight), 
#               method="lm", formula = y ~ boot::inv.logit(x),
#               col="black", lty=2) + 
