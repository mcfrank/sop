...
kail.fun <- function (i, a, b, c) {
  a + b * exp(-c * i)
}

inv.logit <- boot::inv.logit # get inv.logit from boot library

acc.fun <- function(i, alpha, beta) {
  inv.logit(alpha + beta * i)
}
```
...
ages <- seq(0,20,.1)
d <- data.frame(x = ages,
                y = kail.fun(ages, a = a.samp, b = b.samp, c = c.samp))
...
sims <- expand.grid(n = ns, 
  ...
  do(data.frame(rt = rlnorm(nsims, 
                            meanlog = kail.fun(.$age - 1,
                                               a.samp, b.samp, c.samp))*.$n, 
                success = rbinom(nsims, .$n, 
                                 acc.fun(alpha.samp, beta.samp, 
                                         .$age)) == .$n)) %>%
  mutate(relevant = rt < theta & success) %>%
  group_by(n, age, theta) %>%
  summarise(p = mean(relevant)) %>%
...
```{r}
d <- read.csv("../data/word_recognition_MA.csv") %>%
...
ggplot(d.kid, aes(x = age_years, y = x_1)) + 
