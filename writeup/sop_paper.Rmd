...
```{r}
d_wr <- read.csv("../data/word_recognition_MA.csv") %>%
...
ggplot(d_gf, aes(x = age_years, y = m_rt_seconds)) + 
  geom_point(aes(size = n_1, col = short_cite)) + 
...
ggplot(d_gf, aes(x = age_years, y = x_1)) + 
  geom_point(aes(size = n_1, col = pointing)) + 
  geom_line(data = d_gf_pred, aes(x = age_years, y = acc_pred, col = pointing), 
            lty = 2) + 
  # geom_hline(yintercept=.5, lty=3) +  
  ylim(c(0,1)) + 
  xlim(c(0,5)) + 
...
ggplot(ms, aes(x = age, y = 1/p)) + 
  ...
  scale_colour_solarized(guide=FALSE) + 
  ylab("Number of learning instances needed for \"fast mapping\"") + 
  xlab("Age (months)") + 
